...
def write_example(outdir, label, idx, elem, terms):
    xml_path = outdir / f"{label}_trait_example_{idx}.xml"
    summary_path = outdir / f"{label}_trait_example_{idx}_summary.tsv"
    ...
    with summary_path.open("w", encoding="utf-8") as out:
        ...
        for tag, attrs, value in summarize_trait(elem):
...
def main():
    parser = argparse.ArgumentParser(description="Extract phenotype-matching Trait blocks from pinned GTR XML.")
    parser.add_argument("--xml", default="/mnt/storage/gtr/gtr_ftp.xml")
    parser.add_argument("--outdir", default="results/reports/gtr_schema_exploration/trait_examples")
    ...
    index_path = outdir / "trait_example_index.tsv"
