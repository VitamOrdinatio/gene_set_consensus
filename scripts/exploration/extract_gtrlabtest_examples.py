...
def elem_text(elem):
    return normalize_text(" ".join(t for t in elem.itertext() if t))
...
def summarize_block(elem):
    ...
    for child in elem.iter():
        ...
        if text:
            rows.append((tag, text[:500]))
    return rows

def write_example(outdir, label, idx, elem, terms):
    xml_text = serialize(elem)
    text = elem_text(elem)
    matches = matched_terms(text, terms)

    xml_path = outdir / f"{label}_example_{idx}.xml"
    summary_path = outdir / f"{label}_example_{idx}_summary.tsv"

    xml_path.write_text(xml_text, encoding="utf-8")

    with summary_path.open("w", encoding="utf-8") as out:
        out.write("tag\ttext\n")
        for tag, value in summarize_block(elem):
...
def main():
    parser = argparse.ArgumentParser(description="Extract representative GTRLabTest XML blocks for schema reconnaissance.")
    parser.add_argument("--xml", default="/mnt/storage/gtr/gtr_ftp.xml")
    parser.add_argument("--outdir", default="results/reports/gtr_schema_exploration/gtrlabtest_examples")
